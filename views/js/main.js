...
var resizePizzas = function(size) { 
  ...
  var theWidth = document.querySelector("#randomPizzas").offsetWidth; //since this value is always same, took out of the loop
  ...
  //modified the loop to speed the rendering by moving out variable allClasses
  //and calculation of newWidth out of the loop
  function changePizzaSizes(size) {
    var allClasses = document.querySelectorAll(".randomPizzaContainer");
    var dx = determineDx(allClasses[0], size);
    var newWidth = (allClasses[0].offsetWidth + dx) + 'px';

    for (var i = 0; i < allClasses.length; i++) {
        allClasses[i].style.width = newWidth; 
    }
  }
...
for (var i = 2; i < 100; i++) {
  ...
  //console.log(pizzasDiv+" this is pizzasDiv")
...
function updatePositions() {
  ...
  var items = document.getElementsByClassName('mover');//changed querySelector to getElements commmand(improve speed)
  var leng_th = items.length; //moved the value of the loop since it is same value
  var theTop = document.body.scrollTop;//moved out of the loop
  var phase =[];
  
  for (var i = 0; i <5; i++) {  //moved out of the loop since it doesnt need to be re-calculated for each loop
    var temp = Math.sin((theTop / 1250) + (i % 5));
    phase[i]= temp;
  }
